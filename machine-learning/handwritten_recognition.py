...
用法:
  python3 08_handwrite_recognition.py <data_dir> <num_classes>
  # 训练新模型
  
  python3 08_handwrite_recognition.py --load_model model.pth
  # 加载模型进行推理
"""
...
class HandwriteDataset(Dataset):
    ...
    Annotation file format (each class one .json file):
      {
        "class": "A",
        "images": [
          {"path": "img1.jpg", "points": [[x1,y1,x2,y2]]}
        ]
      }
    """

    def __init__(self, root_dir, num_classes, transform=None, target_transform=None):
        ...
        # 加载所有类别信息
        self.class_to_idx = {class_name: idx for idx, class_name in enumerate(DEFAULT_CLASSES)}
    ...
    def _generate_synthetic_dataset(self):
        ...
        for class_idx, class_name in enumerate(DEFAULT_CLASSES[:self.num_classes]):
            class_dir = os.path.join(self.root_dir, class_name)
    ...
    def _draw_letter(self, draw, img_size, letter):
        ...
        # 简单的字母形状绘制 (简化版)
        if letter == 'A':
            draw.line([offset, offset*5, offset*6, offset*5], fill='black', width=2)
            draw.line([offset, offset, offset, offset*5], fill='black', width=2)
            draw.line([offset*6, offset, offset*6, offset*5], fill='black', width=2)
            draw.line([offset*2, offset*3, offset*4, offset*5], fill='black', width=2)
            ...
            draw.rectangle([offset, offset, offset*3, offset*6], outline='black', width=2)
            draw.ellipse([offset*2, offset, offset*5, offset*5], outline='black', width=2)

        elif letter == 'E':
            draw.rectangle([offset, offset, offset*6, offset*6], outline='black', width=2)
            draw.line([offset, offset*3, offset*4, offset*3], fill='black', width=2)
            draw.line([offset, offset*5, offset*4, offset*5], fill='black', width=2)
            ...
            draw.rectangle([offset, offset, offset*6, offset*6], outline='black', width=2)
            draw.arc([offset*4, offset*4, offset*6, offset*6], start=180, end=360, fill='black', width=2)

        elif letter == 'H':
            draw.rectangle([offset, offset, offset*3, offset*6], outline='black', width=2)
            draw.rectangle([offset*3, offset, offset*6, offset*3], outline='black', width=2)
            draw.line([offset*3, offset*3, offset*3, offset*6], fill='black', width=2)

        elif letter == 'I':
            draw.line([offset*3, offset, offset*3, offset*6], fill='black', width=2)

        elif letter == 'J':
            draw.arc([offset*2, offset, offset*5, offset*4], start=180, end=360, fill='black', width=2)
            draw.line([offset*4, offset*4, offset*4, offset*6], fill='black', width=2)

        elif letter == 'K':
            draw.rectangle([offset, offset, offset*3, offset*6], outline='black', width=2)
            draw.line([offset*3, offset, offset, offset*3], fill='black', width=2)
            draw.line([offset*3, offset*3, offset*3, offset*6], fill='black', width=2)

        elif letter == 'L':
            draw.rectangle([offset, offset, offset*3, offset*6], outline='black', width=2)
            draw.line([offset, offset*6, offset*3, offset*6], fill='black', width=2)

        elif letter == 'M':
            draw.line([offset, offset, offset, offset*6], fill='black', width=2)
            draw.line([offset*3, offset, offset*3, offset*6], fill='black', width=2)
            draw.line([offset*6, offset, offset*6, offset*6], fill='black', width=2)
            ...
            draw.ellipse([offset, offset, offset*6, offset*6], outline='black', width=2)

        elif letter == 'P':
            draw.rectangle([offset, offset, offset*3, offset*6], outline='black', width=2)
            draw.ellipse([offset*2, offset, offset*4, offset*3], outline='black', width=2)

        elif letter == 'Q':
            draw.ellipse([offset, offset, offset*6, offset*6], outline='black', width=2)
            draw.line([offset*4, offset, offset*6, offset*2], fill='black', width=2)
            ...
            draw.arc([offset, offset, offset*6, offset*3], start=0, end=180, fill='black', width=2)
            draw.arc([offset, offset*3, offset*6, offset*6], start=180, end=360, fill='black', width=2)

        elif letter == 'T':
            draw.line([offset, offset*5, offset*6, offset*5], fill='black', width=2)
            draw.line([offset*3, offset, offset*3, offset*5], fill='black', width=2)

        elif letter == 'U':
            draw.rectangle([offset, offset, offset*3, offset*6], outline='black', width=2)
            draw.rectangle([offset*3, offset*4, offset*6, offset*6], outline='black', width=2)

        elif letter == 'V':
            draw.line([offset, offset*5, offset*3, offset], fill='black', width=2)
            draw.line([offset*3, offset, offset*6, offset*5], fill='black', width=2)

        elif letter == 'W':
            draw.line([offset, offset*5, offset*2, offset], fill='black', width=2)
            draw.line([offset*2, offset, offset*4, offset*5], fill='black', width=2)
            draw.line([offset*4, offset*5, offset*6, offset], fill='black', width=2)

        elif letter == 'X':
            draw.line([offset, offset, offset*6, offset*6], fill='black', width=2)
            draw.line([offset, offset*6, offset*6, offset], fill='black', width=2)

        elif letter == 'Y':
            draw.line([offset, offset, offset, offset*3], fill='black', width=2)
            draw.line([offset*3, offset*3, offset*3, offset*6], fill='black', width=2)
            draw.line([offset*3, offset*3, offset*6, offset*5], fill='black', width=2)

        elif letter == 'Z':
            draw.line([offset, offset, offset*6, offset], fill='black', width=2)
            draw.line([offset*6, offset, offset*6, offset*6], fill='black', width=2)
            draw.line([offset, offset*6, offset*6, offset*6], fill='black', width=2)
...
class HandwriteCNN(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(256 * 4 * 4, 512),  # 图像尺寸 64x64，经过 4 次池化 -> 4x4
            nn.ReLU(),
            nn.Dropout(0.5),
...
def train_handwrite_model(data_dir, num_classes, config=DEFAULT_CONFIG):
    ...
    test_dataset = torch.utils.data.TensorDataset(
        torch.stack([sample[0] for sample in test_samples]),
        torch.tensor([sample[1] for sample in test_samples])
    )
